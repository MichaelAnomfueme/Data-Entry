...
def save_result(candidate_name, candidate_exam_no, total_questions, attempted_questions,
                unattempted_questions, failed_questions, final_score, grade) -> None:
    try:
...
async def handle_response(update: Update, context: CallbackContext) -> int:
    ...
    if not candidate_name:
        ...
        await update.message.reply_text(
            f"Hi {candidate_name}! Welcome to the {TEST_NAME} test. You have {len(QUESTIONS)} questions. "
...
async def start_test(update: Update, context: CallbackContext) -> int:
    ...
    if query.data == 'START':
        ...
        context.user_data['end_time'] = time.time() + TIME_LIMIT
...
async def ask_question(update: Update, context: CallbackContext) -> int:
    ...
    if remaining_time < (TIME_LIMIT / 100) * 50:
        if remaining_time < 60:
            await update.effective_message.reply_text(f"Hurry up you have {int(remaining_time)} seconds left\n")
        elif remaining_time < 3600:
            await update.effective_message.reply_text(
                f"Hurry up you have {int(remaining_time / 60)} minutes and {int(remaining_time % 60)} seconds left\n")
    ...
    if question_number < len(QUESTIONS):
        ...
        await update.effective_message.reply_text(f"Question {question_number + 1}: {question}", reply_markup=reply_markup)
        return ASK_QUESTION
    else:
        return await end_test(update, context)
